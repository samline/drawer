...
2. **Open** — call `drawer.setOpen(true)` or `openDrawer(id?)`. The overlay and dialog mount with `data-state="open"`, modal scroll ownership is acquired, and focus moves to the dialog. Set `autoFocus: true` to focus its first visible control instead.
