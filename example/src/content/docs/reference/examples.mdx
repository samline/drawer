...
These examples target `@samline/drawer@3.0.0-beta.4` and its lazy-Presence lifecycle. Each registered id owns a host; overlay/content exist only while open or exiting.
...
Snap points let the user drag between predefined positions. Numbers are fractions of the viewport or custom container (`0.5` is 50%). Strings are parsed as absolute pixel counts (`'180px'` is 180); percent-suffixed strings do not use percentage math in beta.4.
...
For a `<script>` integration, call `window.Drawer.createDrawer` on mount and `window.Drawer.destroyDrawer` on teardown. For a bundler, use the [root named registry helpers](/drawer/getting-started/#registry-helpers-bundler).
