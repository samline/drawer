...
<Tabs syncKey="runtime">
	...
	<TabItem label="Browser global">
		...
		No bundler required. Drop the CDN script and use `window.Drawer.createDrawer(options)`.
...
Pair this with the [browser registry helpers](/drawer/getting-started/#browser-registry-helpers-bundler) when loading from a `<script>` tag: call `window.Drawer.createDrawer` on show, `window.Drawer.destroyDrawer` on close.
